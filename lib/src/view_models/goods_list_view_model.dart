  ...
  favorite(List<Goods> goodsList, int i) async {
    if(super.user == null){
      ...
      if(!super.busy){
        ...
        bool isCollection = goods.hasCollection(super.user.userid);
        if(isCollection){
          process = _goodsService.unfavorite(super.user.userid, goods.productId);
          diff = -1;
        }else{
          process = _goodsService.favorite(super.user.userid, goods.productId);
          diff = 1;
        }
        ...
        if(res.code == 0){
          ...
          goodsList[i] = goods;
          goods.faceUserId = super.user.userid;
          super.user.collectionTotal += diff;
          notifyListeners();
        }
        super.setBusy();
      }
