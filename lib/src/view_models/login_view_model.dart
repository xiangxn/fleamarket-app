...
class LoginViewModel extends BaseViewModel implements TickerProvider{
  ...
  updateUser(User user) async {
    if(_accountService.updateUser(user)){
  ...
  login() async {
    if(_loginFormKey.currentState.validate()){
      ...
      var process = _accountService.login(_phone, _password);
  ...
  register() async {
    if(_registerFormKey.currentState.validate()){
      ...
      super.loading();
      ExtResult recommendedRes = await checkRecommended();
      if(recommendedRes.code == 0){
        ExtResult res = await _accountService.register(_phone, _password, _vcode, _recommended);
        super.loading();
        if(res.code == 0){
          updateUser(res.data);
        }else{
        ...
        super.loading();
        super.toast(recommendedRes.msg);
      }
    }
  }

  Future<ExtResult> checkRecommended() async {
    if(_recommended == null || _recommended.length == 0){
      return ExtResult(code: 0);
    }else{
      ExtResult res = await _accountService.validateReferral(_recommended);
      return res;
    }
  }
  ...
  sendVcode() async {
    if(_phoneKey.currentState.validate() && !super.busy){
      ...
      print('_phone ${_phone}');
      super.setBusy();
      var process = _accountService.sendVcode(_phone);
