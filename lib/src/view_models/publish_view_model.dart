...
class PublishViewModel extends BaseViewModel {
  ...
  get prices {
    ...
    String freight =
        _freightAmount == 0 ? super.locale.translation('publish.none_freight') : super.locale.translation('publish.freight', [_freightAmount.toString()]);
  ...
  submit() async {
    ...
    _goods.price = '${this.pricingAmount} ${this._symbol}';
    _goods.postage = '${this.freightAmount} ${this._symbol}';
    ...
    } else if (_goods.position == null || _goods.position.isEmpty) {
    ...
    if (this._isUpdate) {
      ...
      Map product = {
        ...
        "position": _goods.position,
        //"release_time": DateTime.now().toString().split(".")[0].replaceFirst(" ", "T")
        "release_time": DateTime.now().toIso8601String()
      };
      final result = await _goodsService.publishProduct(accountService.activeKey, userEosId, userId, product, _photos);
      loading();
      if (result) {
        pop();
      } else {
